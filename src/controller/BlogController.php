...
class BlogController extends CrudController
{
    ...
    public function getData()
	{
		...
		$this->tableProp["extraActions"] = array(
            array("button" => "u-btn-info", "name" => "btnGrid", "function" => "getImageList", "icon" => "fa fa-file-image-o", "title" => "Agrega imágenes",  "params" => array("modTitle" => "Blog imagen", "copyBtn" =>true)),
            array("button" => "u-btn-info", "name" => "btnText", "function" => "getTextEditor", "icon" => "fa fa-file-text-o",  "title" => "Agrega Texto",  "params" => array("modTitle" => "Blog file")),

		);

		//Traemos el modelo
        $this->model = $this->getModel();   	
        $this->tableName = "SITE_BLOG";
   ...
   */
   private function detailSave()
	{
		...
		$nombre = PwFunciones::getIdValue($this->connection, "SITE_BLOG", "ID", $keyParams["ID"], "TITULO");
		...
		$datos = array("TEXTO" => "", "TEXTO_EN" => "" );

		if($editorParams)
		{
			
			$datos["TEXTO"] =  rawurlencode($editorParams["editor_es"]);						
			$datos["TEXTO_EN"] =  rawurlencode($editorParams["editor_en"]);			
		}

	//	PwFunciones::getVardumpLog($datos);

		$keys = array ("ID" => $id);		
		$result = PwSql::updateData($this->connection, "SITE_BLOG", $datos, $keys);
	...
	*/
	private function getDetail($saveAction = null)
	{
		...
		$fields = array("TEXTO", "TEXTO_EN", "TITULO");
		$sqlResult = PwSql::executeQuery($this->connection, "SITE_BLOG", $fields, $condition);
		$texto = $texto3 = "";
		$textoEn = $textoEn3 = "";
		...
		if($sqlResult)
		{
			...
			$texto = rawurldecode($sqlItem["TEXTO"]);
		
			$textoEn = rawurldecode($sqlItem["TEXTO_EN"]);
			$label = rawurldecode($sqlItem["TITULO"]);
		}
