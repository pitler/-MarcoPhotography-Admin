...
class BaseController //extends ArrayGrid
{
       ...
       if($this->tableProp["cols"] == 3)
       {
   		...
   		if($this->model)
		{
			...
			foreach ($this->model  as $key=> $modelParams) 
			{
				...
				//Si no es editable, lo quitamos del form
	...
	protected function getList($resultFlag = false)
    {
		...
		if($sqlResults)
		{
			...
			{		
				...
				//Para uodate, delete o acciones extra
				if($this->tableProp["update"] == true || $this->tableProp["delete"] == true || $this->tableProp["extraActions"])
				{
					...
					if(isset($this->tableProp["extraActions"]) && is_array($this->tableProp["extraActions"]))
					{
						...
						foreach($this->tableProp["extraActions"] as $action)
						{
							...
							$acciones = preg_replace("/__TITLE__/", $sqlItem[$action["title"]], $acciones);
							$acciones = preg_replace("/__KEYS__/", $kv, $acciones);
    ...
    {    	
		...
		foreach ($this->model as $modelItem)
		{
			...
			if($modelItem["type"] == "datepicker")
			{
        		...
        		{
            		$value = date_create($value);
           			if(isset($modelItem["format"]) && $modelItem["format"] != null)
           			{
               			$value =  date_format($value,  $modelItem["format"]);                      
           			}
            			//Usamos el formato de default que se define al inicio de la clase
           			else
           			{
...
$template["tdUpdate"] = <<< TEMP
 <a href="#"   class="btn btn-sm u-btn-blue" id = "btnEdit" name = "btnEdit" onclick="getModal('__KEYS__');return false;">
	<i class="hs-admin-pencil"></i>
</a> 
TEMP;

$template["tdDelete"] = <<< TEMP
 <a href="#"   class="btn btn-sm u-btn-primary" id = "btnDel" name = "btnDel" onclick="doDelete('__KEYS__');return false;">
	<i class="hs-admin-trash"></i>
</a> 
TEMP;

$template["tdExtra"] = <<< TEMP
 <a href="#"   class="btn btn-sm __EXTRABTNBUTTON__" id = "__BTNNAME__" name = "__BTNNAME__" onclick="__EXTRAFN__('__KEYS__', '__TITLE__');return false;">
