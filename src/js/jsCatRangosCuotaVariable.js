$(document).ready(function() {
    ...
    $(document).on('click', '.btn_save', function(event) {
        ...
        params.porcentaje = arr.fporcentaje;
        params.puntosBase = arr.fpuntosBase;
        params.id = id;
        params.rango = rango;
 ...
 * @param {*} title 
 */
function rangosData(keyParams, title) {

    $("#detailModalTitle").html("");
    $("#detailModalTitle").html(title);
    $('#detailModal').modal('show');
    getDetailData(detailList, keyParams);
    return false;
 ...
 * @param {*} params 
 */
function getDetailData(action, keyParams, params = null) {
    $.ajax({
        url: "JqController.php",
        type: "POST",
        async: true,
        //dataType: "json",
        data: { "class": controller, "mode": action, "keyParams": keyParams, "params": params, "encrypt": 2 },
